export default function ClubPage() {
  return (
    <div className="mx-auto max-w-2xl px-5 py-16 text-center">
      <span className="inline-block bg-[#fdeee5] text-[#e8530c] text-xs font-bold px-3 py-1 rounded-full mb-6 tracking-wide uppercase">
        Club TMaxTech
      </span>
      ...
      <p className="text-sm text-ink-faint mb-10">
        בחינם. בלי ספאם. ביטול בלחיצה אחת.
      </p>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl border border-rule bg-paper text-ink placeholder:text-ink-faint focus:outline-none focus:border-signal text-right"
            />
          </div>
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl border border-rule bg-paper text-ink placeholder:text-ink-faint focus:outline-none focus:border-signal"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-ink-soft mb-2">מה מעניין אותך?</label>
            <div className="flex flex-wrap gap-2 justify-end">
              {["טלוויזיות", "שואבי אבק", "סאונד ואוזניות", "מחשבים", "הכל"].map((cat) => (
                <button
                  key={cat}
                  className="px-4 py-2 rounded-full border border-rule text-sm text-ink-soft hover:border-signal hover:text-signal transition-colors"
                >
                  {cat}
                </button>
              ))}
      ...
      {/* Telegram CTA */}
      <div className="border border-rule rounded-2xl p-6 flex items-center gap-4">
        ...
        <div className="text-right flex-1">
          ...
          href="https://t.me/TechPulseAlertsBot"
          target="_blank"
          rel="noopener noreferrer"
          className="flex-shrink-0 px-4 py-2 bg-[#229ED9] text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-opacity"
        >
          הצטרף
        </a>
      </div>
