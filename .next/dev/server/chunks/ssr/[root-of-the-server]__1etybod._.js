module.exports = [
"[externals]/next/dist/shared/lib/no-fallback-error.external.js [external] (next/dist/shared/lib/no-fallback-error.external.js, cjs)", ((__turbopack_context__, module, exports) => {

const mod = __turbopack_context__.x("next/dist/shared/lib/no-fallback-error.external.js", () => require("next/dist/shared/lib/no-fallback-error.external.js"));

module.exports = mod;
}),
"[project]/Desktop/techpulse-site/app/favicon.ico (static in ecmascript, tag client)", ((__turbopack_context__) => {

__turbopack_context__.v("/_next/static/media/favicon.2vob68tjqpejf.ico" + (globalThis["NEXT_CLIENT_ASSET_SUFFIX"] || ''));}),
"[project]/Desktop/techpulse-site/app/favicon.ico.mjs { IMAGE => \"[project]/Desktop/techpulse-site/app/favicon.ico (static in ecmascript, tag client)\" } [app-rsc] (structured image object, ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "default",
    ()=>__TURBOPACK__default__export__
]);
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$app$2f$favicon$2e$ico__$28$static__in__ecmascript$2c$__tag__client$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/app/favicon.ico (static in ecmascript, tag client)");
;
const __TURBOPACK__default__export__ = {
    src: __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$app$2f$favicon$2e$ico__$28$static__in__ecmascript$2c$__tag__client$29$__["default"],
    width: 256,
    height: 256
};
}),
"[project]/Desktop/techpulse-site/lib/articles.json.[json].cjs [app-rsc] (ecmascript)", ((__turbopack_context__, module, exports) => {

module.exports = JSON.parse("[{\"slug\":\"אפל-משדרגת-את-safari-כלי-פיתוח-חדש-שיהפוך-את-בדיקת-האתרים-למ\",\"headline\":\"אפל משדרגת את Safari: כלי פיתוח חדש שיהפוך את בדיקת האתרים למהיר וקל יותר\",\"dek\":\"📱 אפל משדרגת את Safari: כלי פיתוח חדש שיהפוך את בדיקת האתרים למהר וקל יותר\\n\\nאפל הטמיעה שרת MCP חדש בדפדפן Safari שמאפשר לסוכני קוד לבדוק אתרים ישירות מהדפדפן\\n\\nעיקרי הפרטים:\\n• כלי ז\",\"category\":\"events\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Events\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>אפל משדרגת את Safari: כלי פיתוח חדש שיהפוך את בדיקת האתרים למהר וקל יותר</p>\\n<p>Apple is introducing a new MCP server for Safari that lets coding agents inspect websites directly in the browser, giving them access to page content, console logs, network requests, screenshots, and more. Here are the details.</p>\\n<h2>הנקודות המרכזיות</h2>\\n<p>- אפל הטמיעה שרת MCP חדש בדפדפן Safari שמאפשר לסוכני קוד לבדוק אתרים ישירות מהדפדפן</p>\\n<p>- כלי זה נותן גישה למידע מסך, יומני console, בקשות רשת וצילומי מסך בלי צורך בכלים חיצוניים</p>\\n<p>- השדרוג הזה מיועד בעיקר למפתחים ולאנשי tech, אך משקף את המגמה של אפל להפוך את הדפדפן לסביבת עבודה חזקה יותר</p>\\n<p>### זווית נוספת</p>\\n<p>אפל משיקה תכונה חדשה שתחסוך הרבה זמן למפתחים</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>אפל משדרגת את Safari: כלי פיתוח חדש שיהפוך את בדיקת האתרים למהר וקל יותר זהו עדכון שכדאי לעקוב אחריו מקרוב, במיוחד אם אתם שוקלים רכישה בקרוב.</p>\\n<p>קרא את הכתבה המלאה ולמד איך זה משנה את משחק בדיקת האתרים</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא ועדכני של כל מה שקורה בעולם האלקטרוניקה והטכנולוגיה — ביקורות, השוואות מחירים, ומדריכי קנייה.</p>\\n<p>*פורסם: 02/07/2026 16:08*</p>\\n<p>🔔 **הצטרף ל-Club TMaxTech** — אלפי חברים שמקבלים דילים בלעדיים ישירות מהיבואנים. בחינם, בלי ספאם.</p>\\n<p>👉 [הצטרף עכשיו](/club)</p>\",\"score\":6.57},{\"slug\":\"האייפד-פרו-2027-עומד-להשתיל-את-הטכנולוגיה-החדשה-של-אייפון-17\",\"headline\":\"האייפד פרו 2027 עומד להשתיל את הטכנולוגיה החדשה של אייפון 17 פרו מקס — אבל יש תמורה שאתם צריכים לדעת עליה\",\"dek\":\"📱 \\n\\niPad Pro הבא יקבל את אותו מעבד חדש שמופיע ב-iPhone 17 Pro Max — ביצועים שלא ראינו עדיין\\n\\nעיקרי הפרטים:\\n• התשובה למשאלה: כן, זה יהיה שווה, אבל המחיר יעלה בהתאם\\n• ההבדל מהדור הנו\",\"category\":\"deals\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Deals\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>האייפד פרו 2027 עומד להשתיל את הטכנולוגיה החדשה של אייפון 17 פרו מקס — אבל יש תמורה שאתם צריכים לדעת עליה</p>\\n<p>Apple is reportedly testing out four new iPad models for its spring 2027 refresh that will focus on improving performance.</p>\\n<h2>הנקודות המרכזיות</h2>\\n<p>- iPad Pro הבא יקבל את אותו מעבד חדש שמופיע ב-iPhone 17 Pro Max — ביצועים שלא ראינו עדיין</p>\\n<p>- התשובה למשאלה: כן, זה יהיה שווה, אבל המחיר יעלה בהתאם</p>\\n<p>- ההבדל מהדור הנוכחי יהיה דרמטי — מהירות עיבוד וטיפול בתוכן כבד</p>\\n<p>### זווית נוספת</p>\\n<p>הטאבלט שצפיתם לו: iPad Pro החדש עם המעבד שמשנה כל קו משחק</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>האייפד פרו 2027 עומד להשתיל את הטכנולוגיה החדשה של אייפון 17 פרו מקס — אבל יש תמורה שאתם צריכים לדעת עליה זהו עדכון שכדאי לעקוב אחריו מקרוב, במיוחד אם אתם שוקלים רכישה בקרוב.</p>\\n<p>קרא את ההשוואה המלאה — וברר אם זה יהיה הטאבלט שלך</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא ועדכני של כל מה שקורה בעולם האלקטרוניקה והטכנולוגיה — ביקורות, השוואות מחירים, ומדריכי קנייה.</p>\\n<p>*פורסם: 02/07/2026 16:08*</p>\\n<p>🔔 **הצטרף ל-Club TMaxTech** — אלפי חברים שמקבלים דילים בלעדיים ישירות מהיבואנים. בחינם, בלי ספאם.</p>\\n<p>👉 [הצטרף עכשיו](/club)</p>\",\"score\":8.36},{\"slug\":\"גוגל-השיקה-מחולל-תמונות-ai-שמייצר-ביניה-בשניות-האם-זה-משנה-א\",\"headline\":\"גוגל השיקה מחולל תמונות AI שמייצר ביניה בשניות — האם זה משנה את המשחק לעיצובים ודיוור?\",\"dek\":\"📱 \\n\\nטכנולוגיית Nano Banana 2 Lite של גוגל יוצרת תמונות AI בעדכניות של 4 שניות בלבד\\n\\nעיקרי הפרטים:\\n• הטכנולוגיה הופכת את עיצוב גרפי וגודל תוכן לשיגרה שלא דורשת ידע מקצועי\\n• השלכות ע\",\"category\":\"events\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Events\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>גוגל השיקה מחולל תמונות AI שמייצר ביניה בשניות — האם זה משנה את המשחק לעיצובים ודיוור?</p>\\n<p>Nano Banana 2 Lite makes AI images dramatically faster and changes the creative process.</p>\\n<h2>הנקודות המרכזיות</h2>\\n<p>- טכנולוגיית Nano Banana 2 Lite של גוגל יוצרת תמונות AI בעדכניות של 4 שניות בלבד</p>\\n<p>- הטכנולוגיה הופכת את עיצוב גרפי וגודל תוכן לשיגרה שלא דורשת ידע מקצועי</p>\\n<p>- השלכות על עסקים בישראל: מקטינות עלויות עיצוב ויוצרות תחרות חדשה בתעשיית הקריאייטיב</p>\\n<p>### זווית נוספת</p>\\n<p>זה קרה: AI של גוגל יוצר תמונות מעוצבות מהר יותר מכמו שאתה קורא את הכותרת הזו</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>גוגל השיקה מחולל תמונות AI שמייצר ביניה בשניות — האם זה משנה את המשחק לעיצובים ודיוור? זהו עדכון שכדאי לעקוב אחריו מקרוב, במיוחד אם אתם שוקלים רכישה בקרוב.</p>\\n<p>קראו את הסקירה המלאה — מה זה בדיוק וכמה זה משנה עבורכם</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא ועדכני של כל מה שקורה בעולם האלקטרוניקה והטכנולוגיה — ביקורות, השוואות מחירים, ומדריכי קנייה.</p>\\n<p>*פורסם: 02/07/2026 16:08*</p>\\n<p>🔔 **הצטרף ל-Club TMaxTech** — אלפי חברים שמקבלים דילים בלעדיים ישירות מהיבואנים. בחינם, בלי ספאם.</p>\\n<p>👉 [הצטרף עכשיו](/club)</p>\",\"score\":8.57},{\"slug\":\"רמקול-bose-הנייד-שכולם-רוצים-כרגע-בהנחה-של-29-והמחיר-הזה-לא\",\"headline\":\"רמקול Bose הנייד שכולם רוצים כרגע בהנחה של 29% — והמחיר הזה לא יישאר עוד הרבה זמן\",\"dek\":\"📱 \\n\\nBose SoundLink Flex (2nd Gen) ירד ל-99 דולר — זה המחיר הנמוך ביותר השנה\\n\\nעיקרי הפרטים:\\n• עמיד לחלוטין: עמיד למים (IP67) וצף במים — מושלם לחוף וטיולים\\n• קול גדול מרמקול קומפקטי \",\"category\":\"soundbars\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Soundbars\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<p>📱 רמקול Bose הנייד שכולם רוצים כרגע בהנחה של 29% — והמחיר הזה לא יישאר עוד הרבה זמן</p>\\n<p>Bose SoundLink Flex (2nd Gen) ירד ל-99 דולר — זה המחיר הנמוך ביותר השנה</p>\\n<p>עיקרי הפרטים:</p>\\n<p>• עמיד לחלוטין: עמיד למים (IP67) וצף במים — מושלם לחוף וטיולים</p>\\n<p>• קול גדול מרמקול קומפקטי — אדווה מעט מחומרים טעימים</p>\\n<p>בדקו את המחיר באמזון לפני שהוא חוזר ל-139 דולר</p>\\n<p>🔗 קרא עוד: https://www.soundguys.com/bose-soundlink-flex-bluetooth-speaker-2nd-gen-deal-159317/</p>\\n<p>📰 מקור: SoundGuys</p>\\n<p>#סאונדבר #Soundbar #DolbyAtmos #Sonos #TechPulse</p>\",\"score\":6.43},{\"slug\":\"מכירת-הקיץ-של-home-depot-ציוד-גינה-וטרסה-החל-מ-9-דולר-בלבד\",\"headline\":\"מכירת הקיץ של Home Depot: ציוד גינה וטרסה החל מ-9 דולר בלבד\",\"dek\":\"📱 \\n\\nHome Depot משיקה מבצע 4 ביולי עם הנחות משמעותיות על ציוד חיצוני\\n\\nעיקרי הפרטים:\\n• מחירים התחלתיים מ-9 דולר על פריטים בחירה (משוער: 30-35 שקלים בשוק הישראלי)\\n• הזדמנות לעדכון ציו\",\"category\":\"deals\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Deals\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>מכירת הקיץ של Home Depot: ציוד גינה וטרסה החל מ-9 דולר בלבד</p>\\n<p>Home Depot is hosting a huge sale for 4th of July. Check out the best deals I'm shopping right now.</p>\\n<h2>הנקודות המרכזיות</h2>\\n<p>- Home Depot משיקה מבצע 4 ביולי עם הנחות משמעותיות על ציוד חיצוני</p>\\n<p>- מחירים התחלתיים מ-9 דולר על פריטים בחירה (משוער: 30-35 שקלים בשוק הישראלי)</p>\\n<p>- הזדמנות לעדכון ציוד גן וטרסה לעונת הקיץ במחיר תחרותי</p>\\n<p>### זווית נוספת</p>\\n<p>מכירת קיץ בHome Depot: הגיע הזמן לעדכן את הגן שלכם</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>מכירת הקיץ של Home Depot: ציוד גינה וטרסה החל מ-9 דולר בלבד</p>\\n<p>בדוק את המחירים המקבילים בישראל עוד היום</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא ועדכני של כל מה שקורה בעולם האלקטרוניקה והטכנולוגיה — ביקורות, השוואות מחירים, ומדריכי קנייה.</p>\\n<h2>היכן לקנות?</h2>\\n<p>קישור לרכישה ומחירים: https://www.tomsguide.com/home/home-depots-4th-of-july-sale-starts-now-grills-lawn-gear-and-patio-sets-from-usd9</p>\\n<p>*פורסם: 02/07/2026 16:08*</p>\\n<p>🔔 **הצטרף ל-Club TMaxTech** — אלפי חברים שמקבלים דילים בלעדיים ישירות מהיבואנים. בחינם, בלי ספאם.</p>\\n<p>👉 [הצטרף עכשיו](/club)</p>\",\"score\":8.42},{\"slug\":\"סטנד-ודוק-חדש-של-ugreen-למק-מיני-איך-מוסיפים-אחסון-וחיבורים\",\"headline\":\"סטנד ודוק חדש של Ugreen למק מיני: איך מוסיפים אחסון וחיבורים ב-₪200 בלבד\",\"dek\":\"📱 \\n\\nדוק Ugreen M4 לMac mini מחיר משוך: תחת $55 (כ-₪200) ב-Amazon\\n\\nעיקרי הפרטים:\\n• פתרון מעשי: הוספת יציאות USB, HDMI ואחסון חיצוני למק מיני בכמו כלום\\n• עיצוב מחודש: סטנד המשמש גם ד\",\"category\":\"deals\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Deals\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>סטנד ודוק חדש של Ugreen למק מיני: איך מוסיפים אחסון וחיבורים ב-₪200 בלבד</p>\\n<p>Boost your Apple Mac mini's storage and connectivity — Ugreen's Dock & Stand with SSD enclosure is currently 39% off at Amazon.</p>\\n<h2>הנקודות המרכזיות</h2>\\n<p>- דוק Ugreen M4 לMac mini מחיר משוך: תחת $55 (כ-₪200) ב-Amazon</p>\\n<p>- פתרון מעשי: הוספת יציאות USB, HDMI ואחסון חיצוני למק מיני בכמו כלום</p>\\n<p>- עיצוב מחודש: סטנד המשמש גם דוק — לא עוד קבלים בשולחן</p>\\n<p>### זווית נוספת</p>\\n<p>זה הפתרון שחיפשנו: Ugreen M4 מוסיף למק מיני אחסון וחיבורים בעסקה של המחיר</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>סטנד ודוק חדש של Ugreen למק מיני: איך מוסיפים אחסון וחיבורים ב-₪200 בלבד זהו עדכון שכדאי לעקוב אחריו מקרוב, במיוחד אם אתם שוקלים רכישה בקרוב.</p>\\n<p>בדקו את המחיר הנוכחי ב-Amazon — העסקה הזו לא תחזור כל כך מהר</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא ועדכני של כל מה שקורה בעולם האלקטרוניקה והטכנולוגיה — ביקורות, השוואות מחירים, ומדריכי קנייה.</p>\\n<p>*פורסם: 02/07/2026 16:08*</p>\\n<p>🔔 **הצטרף ל-Club TMaxTech** — אלפי חברים שמקבלים דילים בלעדיים ישירות מהיבואנים. בחינם, בלי ספאם.</p>\\n<p>👉 [הצטרף עכשיו](/club)</p>\",\"score\":8.52},{\"slug\":\"google-gemini-מגיע-ל-mac-עם-יכולת-חדשה-אוטומציה-חכמה-של-קבצי\",\"headline\":\"Google Gemini מגיע ל-Mac עם יכולת חדשה: אוטומציה חכמה של קבצים מקומיים — מה זה אומר לעובדים ב-Israel Tech\",\"dek\":\"📱 \\n\\nGoogle השיקה Gemini Spark לעבודה על מחשבי Mac — עוזר AI שעובד ישירות עם הקבצים שלך ללא העלאה לענן\\n\\nעיקרי הפרטים:\\n• האוטומציה המקומית משמעה: פחות תלות בחיבור אינטרנט וגם פחות חש\",\"category\":\"events\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Events\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>Google Gemini מגיע ל-Mac עם יכולת חדשה: אוטומציה חכמה של קבצים מקומיים — מה זה אומר לעובדים ב-Israel Tech</p>\\n<p>Google today launched Gemini Spark for its macOS desktop app, allowing the AI agent to automate tasks involving local files and desktop workflows.</p>\\n<p>Google announced the update today, adding a dedicated</p>\\n<h2>הנקודות המרכזיות</h2>\\n<p>- Google השיקה Gemini Spark לעבודה על מחשבי Mac — עוזר AI שעובד ישירות עם הקבצים שלך ללא העלאה לענן</p>\\n<p>- האוטומציה המקומית משמעה: פחות תלות בחיבור אינטרנט וגם פחות חשדות על פרטיות הנתונים שלך</p>\\n<p>- יוצר תחרות ישירה עם Apple Intelligence — המסתורי שלה הזה שעדיין לא ממש הגיע לישראל</p>\\n<p>### זווית נוספת</p>\\n<p>Gemini Spark על Mac זה סוף הסיפור של 'האם ה-Apple Intelligence שלי פרטי?' — וגם של המחיר</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>Google Gemini מגיע ל-Mac עם יכולת חדשה: אוטומציה חכמה של קבצים מקומיים — מה זה אומר לעובדים ב-Israel Tech זהו עדכון שכדאי לעקוב אחריו מקרוב, במיוחד אם אתם שוקלים רכישה בקרוב.</p>\\n<p>קרא את הניתוח המלא שלנו — והשווה בין Google לApple לפני שאתה משקיע במחשב החדש</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא ועדכני של כל מה שקורה בעולם האלקטרוניקה והטכנולוגיה — ביקורות, השוואות מחירים, ומדריכי קנייה.</p>\\n<p>*פורסם: 02/07/2026 16:08*</p>\\n<p>🔔 **הצטרף ל-Club TMaxTech** — אלפי חברים שמקבלים דילים בלעדיים ישירות מהיבואנים. בחינם, בלי ספאם.</p>\\n<p>👉 [הצטרף עכשיו](/club)</p>\",\"score\":7.72},{\"slug\":\"אוזניות-skullcandy-במחצית-המחיר-bass-כבד-וסוללה-ל-60-שעות-ב\",\"headline\":\"אוזניות Skullcandy במחצית המחיר: bass כבד וסוללה ל-60 שעות ב-120 שקל בלבד\",\"dek\":\"📱 \\n\\nהנחה של 50% — המחיר יורד מ-240 לכ-120 דולר, זה ההנחה הגדולה של השנה\\n\\nעיקרי הפרטים:\\n• 60 שעות סוללה ו-ANC איכותי — אוזניות שווה כסף בשביל מי שאוהב בס חזק\\n• שאלה קריטית: האם זה מ\",\"category\":\"deals\",\"publishedAt\":\"2026-07-02\",\"heroLabel\":\"Deals\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>אוזניות Skullcandy במחצית המחיר: bass כבד וסוללה ל-60 שעות ב-120 שקל בלבד</p>\\n<p>Amazon just halved the Skullcandy Crusher ANC 2 price to $119.99, making its bass-heavy sound and 60-hour battery hard to ignore.</p>\\n<h2>הנקודות המרכזיות</h2>\\n<p>- הנחה של 50% — המחיר יורד מ-240 לכ-120 דולר, זה ההנחה הגדולה של השנה</p>\\n<p>- 60 שעות סוללה ו-ANC איכותי — אוזניות שווה כסף בשביל מי שאוהב בס חזק</p>\\n<p>- שאלה קריטית: האם זה משמעותי גם בישראל? צריך לבדוק זמינות ומחיר בשקלים</p>\\n<p>### זווית נוספת</p>\\n<p>מחצית הנחה על אוזניות Skullcandy — סוללה ענקית ו-ANC שווה הכסף</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>אוזניות Skullcandy במחצית המחיר: bass כבד וסוללה ל-60 שעות ב-120 שקל בלבד</p>\\n<p>בדוק את הנחה הנוכחית ודע אם זה זמין בישראל — לא תמיד הנחה זו חוזרת</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא ועדכני של כל מה שקורה בעולם האלקטרוניקה והטכנולוגיה — ביקורות, השוואות מחירים, ומדריכי קנייה.</p>\\n<h2>היכן לקנות?</h2>\\n<p>קישור לרכישה ומחירים: https://www.soundguys.com/skullcandy-crusher-anc-2-deal-159312/</p>\\n<p>*פורסם: 02/07/2026 16:08*</p>\\n<p>🔔 **הצטרף ל-Club TMaxTech** — אלפי חברים שמקבלים דילים בלעדיים ישירות מהיבואנים. בחינם, בלי ספאם.</p>\\n<p>👉 [הצטרף עכשיו](/club)</p>\",\"score\":8.93},{\"slug\":\"openai-משחקת-בדרך-של-אפל-למה-אנווידיה-צריכה-להיות-מודאגת\",\"headline\":\"OpenAI משחקת בדרך של אפל: למה אנווידיה צריכה להיות מודאגת?\",\"dek\":\"📱 \\n\\nOpenAI מעתיקה את אסטרטגיית ההשתכלול של אפל — פיתוח טכנולוגיה בבית במקום סמך על ספקים\\n\\nעיקרי הפרטים:\\n• הדרך של אפל ליצור מוצרים ייחודיים ועוצמתיים בעצמה הופכת למודל ליוצרי AI\\n• \",\"category\":\"events\",\"publishedAt\":\"2026-06-30\",\"heroLabel\":\"Events\",\"readMinutes\":2,\"featured\":false,\"bodyHtml\":\"<h2>מה הוכרז?</h2>\\n<p>OpenAI's custom AI chip is less about challenging Nvidia today and more about following Apple's successful strategy of controlling the entire technology stack</p>\\n<h2>למה זה חשוב לך?</h2>\\n<p>OpenAI משחקת בדרך של אפל: למה אנווידיה צריכה להיות מודאגת?</p>\\n<p>- OpenAI מעתיקה את אסטרטגיית ההשתכלול של אפל — פיתוח טכנולוגיה בבית במקום סמך על ספקים</p>\\n<p>- הדרך של אפל ליצור מוצרים ייחודיים ועוצמתיים בעצמה הופכת למודל ליוצרי AI</p>\\n<p>- אנווידיה עלולה להפסיד שוק משמעותי אם חברות tech גדולות יחדלו להזמין שבבים ותבנו חומרה משלהן</p>\\n<h2>מה השלב הבא?</h2>\\n<p>עקבו אחרינו ב-TechPulse לכיסוי מלא של כל מה שקורה בעולם האלקטרוניקה.</p>\\n<p>*פורסם: 30/06/2026 14:18*</p>\",\"score\":3.43}]");
}),
"[project]/Desktop/techpulse-site/lib/articles.ts [app-rsc] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "getAllArticles",
    ()=>getAllArticles,
    "getArticleBySlug",
    ()=>getArticleBySlug,
    "getArticlesByCategory",
    ()=>getArticlesByCategory,
    "getFeaturedArticle",
    ()=>getFeaturedArticle
]);
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$articles$2e$json$2e5b$json$5d2e$cjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/lib/articles.json.[json].cjs [app-rsc] (ecmascript)");
;
function getAllArticles() {
    return [
        ...__TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$articles$2e$json$2e5b$json$5d2e$cjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"]
    ].sort((a, b)=>new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime());
}
function getArticleBySlug(slug) {
    return __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$articles$2e$json$2e5b$json$5d2e$cjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"].find((a)=>a.slug === slug);
}
function getArticlesByCategory(category) {
    return getAllArticles().filter((a)=>a.category === category);
}
function getFeaturedArticle() {
    return getAllArticles().find((a)=>a.featured) ?? getAllArticles()[0];
}
}),
"[project]/Desktop/techpulse-site/components/ArticleCard.tsx [app-rsc] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "default",
    ()=>ArticleCard
]);
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/node_modules/next/dist/server/route-modules/app-page/vendored/rsc/react-jsx-dev-runtime.js [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$client$2f$app$2d$dir$2f$link$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/node_modules/next/dist/client/app-dir/link.react-server.js [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$types$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/lib/types.ts [app-rsc] (ecmascript)");
;
;
;
function formatDate(iso) {
    return new Intl.DateTimeFormat("he-IL", {
        day: "numeric",
        month: "long"
    }).format(new Date(iso));
}
function ArticleCard({ article }) {
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])(__TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$client$2f$app$2d$dir$2f$link$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"], {
        href: `/article/${article.slug}`,
        className: "group block border-b border-rule pb-6 last:border-b-0",
        children: [
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                className: "flex items-center gap-2 mb-2",
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                        className: "tag-bracket text-signal",
                        children: __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$types$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["CATEGORY_LABELS"][article.category]
                    }, void 0, false, {
                        fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                        lineNumber: 18,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                        className: "text-ink-faint text-xs",
                        children: "·"
                    }, void 0, false, {
                        fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                        lineNumber: 21,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                        className: "text-xs text-ink-faint",
                        children: formatDate(article.publishedAt)
                    }, void 0, false, {
                        fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                        lineNumber: 22,
                        columnNumber: 9
                    }, this)
                ]
            }, void 0, true, {
                fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                lineNumber: 17,
                columnNumber: 7
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("h3", {
                className: "font-display text-xl font-bold text-ink leading-snug group-hover:text-signal transition-colors",
                children: article.headline
            }, void 0, false, {
                fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                lineNumber: 26,
                columnNumber: 7
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                className: "mt-2 text-sm text-ink-soft leading-relaxed line-clamp-2",
                children: article.dek
            }, void 0, false, {
                fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                lineNumber: 29,
                columnNumber: 7
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                className: "mt-3 flex items-center gap-3 text-xs text-ink-faint",
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                        children: article.heroLabel
                    }, void 0, false, {
                        fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                        lineNumber: 33,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                        children: "·"
                    }, void 0, false, {
                        fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                        lineNumber: 34,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                        children: [
                            article.readMinutes,
                            " דק' קריאה"
                        ]
                    }, void 0, true, {
                        fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                        lineNumber: 35,
                        columnNumber: 9
                    }, this)
                ]
            }, void 0, true, {
                fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
                lineNumber: 32,
                columnNumber: 7
            }, this)
        ]
    }, void 0, true, {
        fileName: "[project]/Desktop/techpulse-site/components/ArticleCard.tsx",
        lineNumber: 13,
        columnNumber: 5
    }, this);
}
}),
"[project]/Desktop/techpulse-site/app/page.tsx [app-rsc] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "default",
    ()=>HomePage
]);
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/node_modules/next/dist/server/route-modules/app-page/vendored/rsc/react-jsx-dev-runtime.js [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$client$2f$app$2d$dir$2f$link$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/node_modules/next/dist/client/app-dir/link.react-server.js [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$articles$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/lib/articles.ts [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$types$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/lib/types.ts [app-rsc] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$components$2f$ArticleCard$2e$tsx__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/Desktop/techpulse-site/components/ArticleCard.tsx [app-rsc] (ecmascript)");
;
;
;
;
;
function HomePage() {
    const featured = (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$articles$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getFeaturedArticle"])();
    const rest = (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$articles$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getAllArticles"])().filter((a)=>a.slug !== featured?.slug);
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
        className: "mx-auto max-w-6xl px-5 py-10",
        children: [
            featured && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])(__TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$client$2f$app$2d$dir$2f$link$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"], {
                href: `/article/${featured.slug}`,
                className: "group block mb-14",
                children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                    className: "grid md:grid-cols-[1fr_minmax(0,2fr)] gap-8 items-start",
                    children: [
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "order-2 md:order-1",
                            children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                className: "aspect-[4/3] rounded-2xl bg-gradient-to-br from-signal-soft to-paper-raised border border-rule flex items-center justify-center",
                                children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                    className: "tag-bracket text-signal text-sm",
                                    children: __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$types$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["CATEGORY_LABELS"][featured.category]
                                }, void 0, false, {
                                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                    lineNumber: 18,
                                    columnNumber: 17
                                }, this)
                            }, void 0, false, {
                                fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                lineNumber: 17,
                                columnNumber: 15
                            }, this)
                        }, void 0, false, {
                            fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                            lineNumber: 16,
                            columnNumber: 13
                        }, this),
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "order-1 md:order-2",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                    className: "tag-bracket text-signal text-xs",
                                    children: "כתבה ראשית"
                                }, void 0, false, {
                                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                    lineNumber: 24,
                                    columnNumber: 15
                                }, this),
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("h1", {
                                    className: "font-display mt-3 text-3xl sm:text-4xl font-extrabold leading-tight text-ink group-hover:text-signal-deep transition-colors",
                                    children: featured.headline
                                }, void 0, false, {
                                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                    lineNumber: 25,
                                    columnNumber: 15
                                }, this),
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                    className: "mt-4 text-lg text-ink-soft leading-relaxed max-w-xl",
                                    children: featured.dek
                                }, void 0, false, {
                                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                    lineNumber: 28,
                                    columnNumber: 15
                                }, this),
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "mt-5 flex items-center gap-3 text-sm text-ink-faint",
                                    children: [
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                            children: featured.heroLabel
                                        }, void 0, false, {
                                            fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                            lineNumber: 32,
                                            columnNumber: 17
                                        }, this),
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                            children: "·"
                                        }, void 0, false, {
                                            fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                            lineNumber: 33,
                                            columnNumber: 17
                                        }, this),
                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                            children: [
                                                featured.readMinutes,
                                                " דק' קריאה"
                                            ]
                                        }, void 0, true, {
                                            fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                            lineNumber: 34,
                                            columnNumber: 17
                                        }, this)
                                    ]
                                }, void 0, true, {
                                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                    lineNumber: 31,
                                    columnNumber: 15
                                }, this)
                            ]
                        }, void 0, true, {
                            fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                            lineNumber: 23,
                            columnNumber: 13
                        }, this)
                    ]
                }, void 0, true, {
                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                    lineNumber: 15,
                    columnNumber: 11
                }, this)
            }, void 0, false, {
                fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                lineNumber: 14,
                columnNumber: 9
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                className: "grid lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-12",
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("section", {
                        children: [
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("h2", {
                                className: "font-display text-sm font-bold text-ink-faint tracking-wide mb-6 pb-3 border-b-2 border-ink",
                                children: "הכתבות האחרונות"
                            }, void 0, false, {
                                fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                lineNumber: 44,
                                columnNumber: 11
                            }, this),
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                className: "space-y-6",
                                children: rest.map((article)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])(__TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$components$2f$ArticleCard$2e$tsx__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"], {
                                        article: article
                                    }, article.slug, false, {
                                        fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                        lineNumber: 49,
                                        columnNumber: 15
                                    }, this))
                            }, void 0, false, {
                                fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                lineNumber: 47,
                                columnNumber: 11
                            }, this)
                        ]
                    }, void 0, true, {
                        fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                        lineNumber: 43,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("aside", {
                        children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "sticky top-6",
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("h2", {
                                    className: "font-display text-sm font-bold text-ink-faint tracking-wide mb-6 pb-3 border-b-2 border-ink",
                                    children: "לפי קטגוריה"
                                }, void 0, false, {
                                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                    lineNumber: 56,
                                    columnNumber: 13
                                }, this),
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("ul", {
                                    className: "space-y-1",
                                    children: Object.entries(__TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$lib$2f$types$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["CATEGORY_LABELS"]).map(([key, label])=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("li", {
                                            children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])(__TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$client$2f$app$2d$dir$2f$link$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"], {
                                                href: `/category/${key}`,
                                                className: "flex items-center justify-between py-2 px-3 rounded-lg text-sm text-ink-soft hover:bg-signal-soft hover:text-signal-deep transition-colors",
                                                children: [
                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                        children: label
                                                    }, void 0, false, {
                                                        fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                                        lineNumber: 66,
                                                        columnNumber: 21
                                                    }, this),
                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$techpulse$2d$site$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                        className: "text-ink-faint",
                                                        children: "←"
                                                    }, void 0, false, {
                                                        fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                                        lineNumber: 67,
                                                        columnNumber: 21
                                                    }, this)
                                                ]
                                            }, void 0, true, {
                                                fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                                lineNumber: 62,
                                                columnNumber: 19
                                            }, this)
                                        }, key, false, {
                                            fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                            lineNumber: 61,
                                            columnNumber: 17
                                        }, this))
                                }, void 0, false, {
                                    fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                                    lineNumber: 59,
                                    columnNumber: 13
                                }, this)
                            ]
                        }, void 0, true, {
                            fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                            lineNumber: 55,
                            columnNumber: 11
                        }, this)
                    }, void 0, false, {
                        fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                        lineNumber: 54,
                        columnNumber: 9
                    }, this)
                ]
            }, void 0, true, {
                fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
                lineNumber: 42,
                columnNumber: 7
            }, this)
        ]
    }, void 0, true, {
        fileName: "[project]/Desktop/techpulse-site/app/page.tsx",
        lineNumber: 11,
        columnNumber: 5
    }, this);
}
}),
"[project]/Desktop/techpulse-site/app/page.tsx [app-rsc] (ecmascript, Next.js Server Component)", ((__turbopack_context__) => {

__turbopack_context__.n(__turbopack_context__.i("[project]/Desktop/techpulse-site/app/page.tsx [app-rsc] (ecmascript)"));
}),
];

//# sourceMappingURL=%5Broot-of-the-server%5D__1etybod._.js.map